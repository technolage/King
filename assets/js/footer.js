...
async function buildFooter() {
  ...
  if (!footerDiv) {
    console.warn('⚠️ عنصر site-footer غير موجود');
    return;
  }
    ...
    <div class="footer-container">
      ...
      <div class="footer-bottom">
        <button id="backToTopBtn" class="back-to-top" onclick="scrollToTop()" 
                title="العودة إلى الأعلى" style="background: ${primaryColor}">⬆️</button>
    ...
    switch (type) {
      ...
      case 'related':
        if (window.currentCategoryId) {
          query = query.where('category', '==', currentCategoryId).orderBy('date', 'desc').limit(6);
        } else {
          query = query.orderBy('date', 'desc').limit(6);
        }
        break;
